...
class ReferenceEncoderIntegrated(nn.Module):
    def __init__(self, input_hidden_dim, style_dim, gst_dim, att_hidden, n_token, r_factor, debug=0, spkr_embed_size=-1, **kwargs):
    ...
    def forward(self, x, whole_spec_len, spkr_vec, debug=0):
        ...
        """
        N, T_ori, O_dec = x.size(0), x.size(1), x.size(2)
        whole_spec_len = torch.div(whole_spec_len, self.r_factor, rounding_mode='trunc')
        output_ref_enc = x.transpose(1, 2).unsqueeze(1)      # N x 1 x C x T
        ref_out_dict = {}
