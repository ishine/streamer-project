...
import tacotron.module.deprecated_model.model_align_v1.module.guided_att_loss as guided_att
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_input, text_lengths, spkr_vec=None, debug=0, **kwargs):
        N = enc_input.size(0)
        phoneme_emb = self.embedding(enc_input)
        text_mask = torch.arange(0, phoneme_emb.size(1), device=phoneme_emb.device).view(1, -1).expand(N, -1)
        text_mask = torch.lt(text_mask, text_lengths.view(-1, 1).expand(-1, phoneme_emb.size(1))).unsqueeze(1)                     # N x T_enc
...
class TsideAtt(nn.Module):
    ...
    def forward(self, query_vec, key_vec, value_vec, text_lengths, spec_lengths):
        ...
        if hasattr(self, 'mask_maker'):
            spec_lengths = torch.div(spec_lengths, self.r_factor, rounding_mode='trunc').long()
