...
class DecoderRNN(nn.Module):
    ...
    """
    def __init__(self, enc_hidden, dec_hidden, output_size, spkr_embed_size, dropout_p=0.5, debug=0):
        super(DecoderRNN, self).__init__()
        self.H_dec = dec_hidden
        self.O_dec = output_size
        self.num_lstm_layers = 2

        in_lstm_size = enc_hidden + dec_hidden + spkr_embed_size + 1
        ...
        self.out_linear = nn.Linear(
            enc_hidden + 4 * dec_hidden,
            output_size
        )
        self.reset_states()

    def forward(self, input_enc, input_dec, spkr_vec, speed=None, debug=0, **kwargs):
        N = input_enc.size(0)
        ...
        if input_dec is None:
            input_dec = input_enc.data.new().resize_(N, 1, self.O_dec).zero_()
        out_prenet = self.prenet(input_dec)  # N x T_dec x H
        
        context = torch.bmm(self.att_weights.transpose(1, 2), input_enc)  # N x T_dec x 2H
