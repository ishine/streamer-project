...
from tacotron.module.deprecated_model.model_cats_v3.module.CBHG import CBHG
...
class PostProcessor(nn.Module):
    ...
    def forward(self, spec, **kwargs):
        ...
        if lengths is None:
            ...
            lengths = torch.tensor([T for _ in range(N)])
            output = self.CBHG(spec, lengths).contiguous()
            output = self.projection(output)
        else:
            spec_mask = kwargs.get('spec_mask')
            output = self.CBHG(spec, lengths).contiguous()
            output = self.projection(output) * spec_mask.unsqueeze(2)
        return output
