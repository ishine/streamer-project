...
class Encoder(nn.Module):
    """ input: N x T
        ...
        return: N x T x H
    """
    def __init__(self, vocab_size, charvec_dim, hidden_size, dropout_p, lang_vec_dim=0, debug=0, **kwargs):
        super(Encoder, self).__init__()
        self.embedding = nn.Embedding(vocab_size, charvec_dim)
        self.conv = nn.ModuleList([
            nn.Sequential(
                nn.Conv1d(charvec_dim+lang_vec_dim, hidden_size, 5, stride=1, padding=2),
    ...
    def forward(self, enc_input, text_lengths, text_mask=1, debug=0, lang_vec=None, **kwargs):
        phoneme_emb = self.embedding(enc_input)
        if lang_vec is not None:
            lang_vec_expanded = lang_vec.expand(-1, phoneme_emb.size(1), -1) # N T H
            phoneme_emb = torch.cat((phoneme_emb, lang_vec_expanded), 2)
        ...
        if text_lengths is None:
            ...
            output, hiddens = self.biLSTM(output)
            output, _ = rnn.pad_packed_sequence(output, True)   # N x T x 2H
        hiddens = hiddens[0].view(1, 2, output.size(0), self.hidden_size // 2)
        # enc_h_out = hiddens[-1]
        enc_h_out = torch.cat((hiddens[:, 0], hiddens[:, 1]), dim=-1).squeeze(0)
        return phoneme_emb, output, enc_h_out
