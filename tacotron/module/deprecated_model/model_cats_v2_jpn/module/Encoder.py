...
import tacotron.module.deprecated_model.model_cats_v2_jpn.module.guided_att_loss as guided_att
...
class Encoder(nn.Module):
    """ input: N x T
        ...
        return: N x T x H
    """
    def __init__(self, vocab_size, charvec_dim, hidden_size, dropout_p, debug=0, acc_dim=0, **kwargs):
        super(Encoder, self).__init__()
        self.embedding = nn.Embedding(vocab_size, charvec_dim)
        self.conv = nn.ModuleList([
            nn.Sequential(
                nn.Conv1d(charvec_dim + acc_dim, hidden_size, 5, stride=1, padding=2),
    ...
    def forward(self, enc_input, text_lengths, text_mask=1, debug=0, **kwargs):
        ...
        if kwargs.get('accents') is not None:
            phoneme_emb = torch.cat((phoneme_emb, kwargs.get('accents')), dim=-1)
...
class ProsodyPredictorNONAR(nn.Module):
    ...
    def forward(self, text_encoding, text_lengths, seq_prosody_vec=None):
        ...
        autoregressive_input = rnn.pack_padded_sequence(text_encoding, text_lengths.cpu(), True, enforce_sorted=False)
        style_vec_prediction, _ = self.prosody_predictor(autoregressive_input)              # N x T_enc x H
        style_vec_prediction, _ = rnn.pad_packed_sequence(style_vec_prediction, True)
