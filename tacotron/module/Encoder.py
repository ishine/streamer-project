...
class Encoder(nn.Module):
    ...
    def __init__(self, vocab_size, charvec_dim, hidden_size, dropout_p,  debug=0, **kwargs):
        ...
        lang_vec_dim = kwargs.get('lang_vec_dim')

        self.pause_bias = nn.Linear(1, charvec_dim, bias=False)
        self.conv = nn.ModuleList([
            nn.Sequential(
                nn.Conv1d(charvec_dim, hidden_size, 5, stride=1, padding=2, bias=False),
                nn.BatchNorm1d(hidden_size),
                nn.ReLU(),
                nn.Dropout(dropout_p),
            ),
            nn.Sequential(
                nn.Conv1d(hidden_size, hidden_size, 5, stride=1, padding=2, bias=False),
                nn.BatchNorm1d(hidden_size),
                nn.ReLU(),
                nn.Dropout(dropout_p),
            ),
    ...
    def forward(self, enc_input, long_pause, text_lengths, text_mask=1, debug=0, **kwargs):
        phoneme_emb = self.embedding(enc_input)
        lang_vec = kwargs.get('lang_vec')
        ...
        output = (phoneme_emb + self.pause_bias(long_pause)).transpose(1, 2) * text_mask        # N x T x H -> N x H x T
