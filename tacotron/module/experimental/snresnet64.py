...
from tacotron.module.resblocks import Block


class SNResNetProjectionDiscriminator(nn.Module):

    def __init__(self, mel_size, num_features=64, activation=F.relu):
        super(SNResNetProjectionDiscriminator, self).__init__()
        self.num_features = num_features
        self.activation = activation

        self.block1 = utils.spectral_norm(nn.Conv2d(1, num_features, (7, 3), 1, 1))
        ...
        self.l_y = utils.spectral_norm(nn.Conv1d(mel_size, num_features * 16, 1))
    ...
    def forward(self, mel_spec, lin_spec):
        h = lin_spec
        h = self.block1(h)
        h = self.block2(h)
        h = self.block3(h)
        ...
        output = self.l6(h).squeeze(1) + torch.sum(torch.sum(self.l_y(mel_spec), dim=2) * h)
