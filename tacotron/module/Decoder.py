...
class DecoderRNN(nn.Module):
    ...
    """
    def __init__(self, enc_hidden, dec_hidden, output_size, spkr_embed_size, pitch_size=1, pitch_embed_size=0, dropout_p=0.5, debug=0, fluency=0):
    ...
    def forward(self, input_enc, input_dec, spkr_vec, debug=0, **kwargs):
        ...
        if kwargs.get('lang_vec') is not None:
            lang_vec = kwargs.get('lang_vec')
            lang_vec_expanded = lang_vec.expand(-1, out_prenet.size(1), -1)
            context = self.context_linear(torch.cat((context, lang_vec_expanded), 2))
