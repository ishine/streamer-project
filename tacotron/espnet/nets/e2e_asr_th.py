...
from tacotron.espnet.nets import CTCPrefixScore
from tacotron.espnet.nets import CTCPrefixScoreTH
from tacotron.espnet.nets.e2e_asr_common import end_detect
from tacotron.espnet.nets.e2e_asr_common import get_vgg2l_odim
from tacotron.espnet.nets.e2e_asr_common import label_smoothing_dist
...
class E2E(torch.nn.Module):
    ...
    def __init__(self, idim, odim, args):
        ...
        # ctc
        # self.ctc = torch.nn.CTCLoss(odim, args.eprojs, args.dropout_rate)
    ...
    def forward(self, xs_pad, ilens, ys_pad, tlens=None):
        '''E2E forward
        ...
        if self.mtlalpha == 0:
            ...
            loss_ctc = self.ctc(hs_pad, hlens, ys_pad, tlens)
        ...
        return loss_ctc, loss_att, acc, None, None
...
class CTC(torch.nn.Module):
    ...
    def forward(self, hs_pad, hlens, ys_pad, olens=None):
        '''CTC forward
        ...
        '''
        ys = []
        ys_pad = ys_pad + 2      # +2 to match index of G2P and ESPnet
        for j, y in enumerate(ys_pad):
            ys.append(y[:olens[j]-1])   # -1 since the last one is for zero padding

        self.loss = None
        hlens = torch.from_numpy(np.fromiter(hlens, dtype=np.int32)).type_as(ys_pad)
        olens = torch.IntTensor(olens).type_as(ys_pad) - 1

        # zero padding for hs
        ys_hat = self.log_softmax(hs_pad, self.dropout_rate)

        # zero padding for ys
        ys_true = torch.cat(ys)

        # get ctc loss
        # expected shape of seqLength x batchSize x alphabet_size
        ys_hat = ys_hat.transpose(0, 1)
        self.loss = F.ctc_loss(ys_hat, ys_true, hlens, olens, blank=0)

        return self.loss

    def forward_old(self, hs_pad, hlens, ys_pad):
        '''CTC forward
    ...
    def log_softmax(self, hs_pad, dropout=None):
        ...
        if not dropout is None:
            output = F.dropout(hs_pad, p=dropout)
        return F.log_softmax(self.ctc_lo(output), dim=2)
